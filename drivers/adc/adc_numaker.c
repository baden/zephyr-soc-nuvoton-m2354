...
#define DT_DRV_COMPAT nuvoton_numaker2_adc
				     ...
				     const struct adc_channel_cfg *chan_cfg)
{
	...
	// LOG_DBG(
	// 	"adc_numaker_channel_setup/ channel_id=%d"
	// 	, chan_cfg->channel_id
	// );
...
static void adc_numaker_isr(const struct device *dev)
{
	...
	if (!(eadc->STATUS2 & EADC_STATUS2_BUSY_Msk) &&
		...
		// LOG_DBG("ADC ISR channels=0x%08x", channel_mask);
...
static int adc_numaker_init(const struct device *dev)
{
    ...
    // LOG_DBG("channels = %d", cfg->channel_cnt);
	// LOG_DBG("pincfg.state_cnt = %d", cfg->pincfg->state_cnt);
	// LOG_DBG("pincfg.state.pin_cnt = %d", cfg->pincfg->states->pin_cnt);
	// LOG_DBG("pincfg.state.pins.pinmux = 0x%04x", cfg->pincfg->states->pins->pin_mux);
	// 
