...
#define DT_DRV_COMPAT nuvoton_numaker2_fmc
#if 1
#include <string.h>
#include <errno.h>
...
static int flash_numaker_erase(const struct device *dev, off_t offset, size_t len)
{
	...
	uint32_t save_addr = addr;

	LOG_DBG("flash_numaker_erase: offset = %ld, len = %zd", offset, len);

	/* return SUCCESS for len == 0 (required by tests/drivers/flash) */
	if (!len) {
		LOG_ERR("flash_numaker_erase: len = 0");
		return 0;
	}

	/* Validate range */
	if (!flash_numaker_is_range_valid(offset, len)) {
		LOG_ERR("flash_numaker_erase: invalid range");
		return -EINVAL;
	}

	/* check alignment and erase only by pages */
	if (((addr % FMC_FLASH_PAGE_SIZE) != 0) || ((len % FMC_FLASH_PAGE_SIZE) != 0)) {
		LOG_ERR("flash_numaker_erase: invalid alignment");
		return -EINVAL;
	}
	...
	if(addr >= FMC_LDROM_BASE) {
		// LOG_ERR("flash_numaker_erase: LDROM");
		FMC_ENABLE_LD_UPDATE();
	} else {
		// LOG_ERR("flash_numaker_erase: APROM");
...
static int32_t flash_numaker_block_write(uint32_t u32_addr, uint8_t *pu8_data, int block_size)
{
	...
	// LOG_ERR("flash_numaker_block_write: u32_addr = 0x%08x, block_size = %d", u32_addr, block_size);
	// LOG_HEXDUMP_ERR(pu8_data, block_size, "flash_numaker_block_write");
...
static int flash_numaker_write(const struct device *dev, off_t offset, const void *data, size_t len)
{
	...
	/* return SUCCESS for len == 0 (required by tests/drivers/flash) */
	if (!len) {
		LOG_ERR("flash_numaker_write: len = 0");
		return 0;
	}

	/* Validate range */
	if (!flash_numaker_is_range_valid(offset, len)) {
		LOG_ERR("flash_numaker_write: invalid range");
		return -EINVAL;
	}

	/* Validate address alignment */
	if ((addr % flash_numaker_parameters.write_block_size) != 0) {
		LOG_ERR("flash_numaker_write: invalid alignment");
		return -EINVAL;
	}

	/* Validate write size be multiples of the write block size */
	if ((len % block_size) != 0) {
		LOG_ERR("flash_numaker_write: invalid write size");
		return -EINVAL;
	}

	/* Validate offset be multiples of the write block size */
	if ((offset % block_size) != 0) {
		LOG_ERR("flash_numaker_write: invalid offset");
		return -EINVAL;
	}

	#if defined(CONFIG_MULTITHREADING)
	if (k_sem_take(&dev_data->write_lock, K_FOREVER)) {
		LOG_ERR("flash_numaker_write: write lock failed");
		return -EACCES;
	}
	#endif
	...
	FMC_Open();
	if(addr >= FMC_LDROM_BASE) {
		LOG_DBG("flash_numaker_write: LDROM");
		FMC_ENABLE_LD_UPDATE();
	} else {
		LOG_DBG("flash_numaker_write: APROM");
		FMC_ENABLE_AP_UPDATE();
	}

	while (blocks) {
		if (flash_numaker_block_write(addr, pu8_data, block_size)) {
			LOG_ERR("flash_numaker_write: write failed");
			rc = -EIO;
			goto done;
		}
		pu8_data += block_size;
